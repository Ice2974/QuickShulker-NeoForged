...
Codex / Agent 完成阶段性开发任务时，应在 `docs/` 下生成中文阶段报告。

阶段报告文件名格式:

* `stage-<版本号>-p<阶段>.<小阶段>-<标题>.md`

如需要命名示例，可参考 `docs/archive/stages/` 下的文档

阶段报告只记录该阶段修改、验证、风险和待确认项，不替代当前发布文档。

 `docs/archive/stages/` 下的文档是历史报告，不应继续作为当前行为依据

当前版本所有更改完成后，维护者会手动将 `docs/` 的阶段文档迁移至 `docs/archive/stages/`; Agent 不应该主动移到阶段文档，除非维护者要求
