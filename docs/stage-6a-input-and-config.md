...
## 阶段 6A.6 追加记录：QuickOpen 菜单副手交换拦截收紧

之前为了修复“打开菜单时 `F` 键副手交换导致复制 / 幽灵物品”的问题，双平台都把 QuickOpen 菜单期间的 `ClickType.SWAP + offhand button`
以及客户端 `keySwapOffhand` 直接整体拦截。

这个做法能保护“副手宿主不能被 `F` 换走”，但也把不影响宿主的原版副手交换一起吞掉了：

- 打开潜影盒 / 末影箱 / 其他 quick-open 菜单时，悬停普通物品按 `F` 也会被直接取消
- 生存和创造模式下都无法将普通物品切到副手

本次补丁把拦截范围收紧到“只拦截会影响当前宿主的副手交换”。

### 服务端菜单侧

- 点击当前锁定的宿主槽位本身时，仍然拦截所有点击类型
- 只有 `ClickType.SWAP` 才继续判断是否需要拦截
- 如果当前宿主在副手（`HostStorageScope.PLAYER_OFFHAND`），继续拦截 `offhand button`
- 如果 `button` 指向当前宿主热栏位，继续拦截，防止数字键把热栏宿主换走
- 其他 `SWAP` 情况不再一律禁止，允许原版普通物品 `<->` 副手交换

这意味着当宿主在主背包时，“普通槽位 <-> 副手”的 `F` 交换会恢复原版行为；但宿主槽位本身仍然不能被点击。

### 客户端输入侧

- 只有当前 `screen` 是 `AbstractContainerScreen` 且 `menu` 是 `ForgeQuickOpenMenu` / `NeoForgeQuickOpenMenu` 时，才会评估 `F` 键是否需要提前取消
- 如果当前宿主本来就在副手，仍然直接 cancel，防止副手宿主被换走
- 如果鼠标当前悬停的就是宿主槽位，仍然 cancel，防止 `F` 直接把宿主换到副手
- 如果 `hoveredSlot == null`，则通常不再 cancel，把行为交还原版

### 允许与仍然禁止的副手交换

现在允许：

- 打开潜影盒界面时，悬停普通物品按 `F` 切到副手
- 打开末影箱界面时，悬停普通物品按 `F` 切到副手
- 生存和创造模式下，只要操作不影响当前宿主，原版副手交换都可以继续执行

仍然禁止：

- 宿主在副手时，按 `F` 尝试换走副手宿主
- 宿主在热栏时，通过数字键把当前宿主热栏位换走
- 鼠标悬停宿主槽位时按 `F`
- 对宿主槽位的左键、右键、`shift-click`、`Q`、`PICKUP_ALL`、`QUICK_CRAFT` 等危险操作

### 同步与安全

这次只收窄拦截范围，不回退之前的安全修复：

- 宿主仍然不能在菜单打开期间被移动、丢弃、数字键交换或用 `F` 换走
- 拦截危险操作后，服务端仍然会同步当前菜单并强制同步 `player.inventoryMenu`，用于避免副手 / 背包的幽灵物品状态残留

### 本次验证状态

- Forge 1.20.1：已在本地运行 `.\gradlew.bat :forge-1.20.1:compileJava`，通过
- NeoForge 1.21.1：已在本地运行 `.\gradlew.bat :neoforge-1.21.1:compileJava`，通过
- 生存 / 创造实机测试尚未在 Minecraft 内完成，需要人工按本次任务清单继续验证
