...
- Forge 1.20.1 / NeoForge 1.21.1 服务端 handler 接入玩家作用域末影箱 bundling：
  ...
  - 鼠标拿单个末影箱，右键真实空槽，从玩家自己的 `EnderChestInventory` 取出第一组物品放入该空槽。
...
- 当前菜单是潜影盒菜单时，拒绝向潜影盒内容槽执行末影箱 extract，避免把末影箱中取出的潜影盒放进另一个潜影盒内容里。
...
- 从末影箱中取出的潜影盒只允许作为普通物品放到安全空槽，不允许放入潜影盒内容槽。
